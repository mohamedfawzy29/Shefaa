...
const STATUS_MAP: Record<number, { label: string; bg: string; dot: string }> = {
    ...
    3: { label: "Checked-In", bg: "bg-amber-50 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300 border border-amber-200/60 dark:border-amber-800/50", dot: "bg-amber-500 animate-pulse" },
    4: { label: "No Show", bg: "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 border border-slate-200/60 dark:border-slate-700/50", dot: "bg-slate-400" },
};
...
export function AppointmentsTable({ appointments = [], isLoading }: AppointmentsTableProps) {
        ...
        },
        {
            ...
            render: (a) => (
                <span
                    ...
                    title={a?.visitReason || "Routine Consultation"}
                >
                    {a?.visitReason || "Routine Visit"}
                </span>
            ),
        },
        {
            header: "Status",
            render: (a) => {
                const statusNum = typeof a?.status === "number" ? a.status : 0;
                const s = STATUS_MAP[statusNum] || { label: "Scheduled", bg: "bg-blue-50 text-blue-700", dot: "bg-blue-500" };
                return (
    ...
    return (
        <Table
            ...
            data={appointments}
            isLoading={isLoading}
            keyExtractor={(a, idx) => a?.appointmentId || String(idx)}
        />
    );
}
